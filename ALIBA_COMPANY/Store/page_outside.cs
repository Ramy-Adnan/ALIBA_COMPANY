...
{
    public partial class page_outside : DevExpress.XtraEditors.XtraUserControl
    {
        Note note;
        int rowCount;
        int OrderId;
        ...
        public void LoadData()
        {
            using (var dbContext = new ALIBA_COMPANY.AlibaRamyEntities())
            {
                ...
                var data = from order in dbContext.TB_order.AsQueryable().AsNoTracking().Where(item => item.action_id == 2)
                ...
                // تطبيق تنسيق الأرقام في الذاكرة
        ...
        private void Btn_add_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    IN_OUT_STOCK = 2,
                    Text = "اخراج المواد من المخزن"
                };
                Add.ShowDialog();
            }
            catch (Exception ex)
            {
                MessageBox.Show("خطأ في العملية Btn_add_Click", ex.Message, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
            ...
            if (reslt == DialogResult.Yes)
            {
                try
                {
                    ...
                    if (OrderId > 0)
                    {
                        ...
                        {
                            var travelItem = context.TB_order.FirstOrDefault(o => o.order_id == OrderId && o.action_id == 2);
                            ...
                            {
                                var recordsToDeleteTBStr = context.TB_str.Where(o => o.order_id == OrderId && o.action_id == 2).ToList();
                                if (recordsToDeleteTBStr == null)
                                {
                                    MessageBox.Show("خطا في الاتصال TBstr", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    return;
                                }
                                var recordsToDeleteTBDetails = context.TB_details.Where(o => o.order_id == OrderId && o.action_id == 2).ToList();
                                ...
                                var Note = " حذف النقل الخارج " + Wasel;
                                note.AddNote(Note, notifications, "حذف", 5);

                                LoadData();
                                MessageBox.Show("تم حذف الخارج بنجاح", "نجاح", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
        ...
        private void page_outside_Load(object sender, EventArgs e)
        {
            LoadData();
        }
        ...
        private async Task LoadDetailsAsync(int id)
        {
            try
            {
                ...
                dataDetails = await Task.Run(() =>
                {
                    ...
                    {
                        var data = db.TB_str
                           .Where(x => x.order_id == id && x.action_id == 2).AsNoTracking()
            ...
            try
            {
                ...
                if (int.TryParse(Id, out int wasell) && wasell != 0)
                {
                        ...
                        var parameters = new Dictionary<string, object>
                    {
                        ...
                        note = new Note();
                        pages.Notifications notifications = new pages.Notifications();
                        var Note = " طباعة وصل نقل خارج " + Id;
